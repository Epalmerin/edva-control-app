...
export default function AdminAttendancePage() {
  ...
  const accountRecords = useMemo(() => {
    if (selectedAccount === "ALL") return records;

    return records.filter((record) =>
      accountEmployeeIds.includes(record.employee_id)
    );
  }, [records, selectedAccount, accountEmployeeIds]);

  const filteredRecords = useMemo(() => {
    if (typeFilter === "ALL") return accountRecords;

    return accountRecords.filter((record) => record.type === typeFilter);
  }, [accountRecords, typeFilter]);
  ...
  const entries = accountRecords.filter((record) => record.type === "ENTRY").length;

  const completed = accountRecords.filter((record) => record.type === "EXIT").length;
        ...
        <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-5 mb-8">
          ...
          <div className="bg-white rounded-2xl shadow-md p-4 flex flex-col md:flex-row gap-3 md:items-end">
            ...
            <div>
              ...
              <select
                className="block mt-1 px-4 py-3 border rounded-xl bg-white min-w-[240px]"
